...
export function MultiSelect({
    ...
    if (allTargetSelected) {
      ...
      onChange(
        typeof maxSelected === "number"
          ? newSelected.slice(0, maxSelected)
          : newSelected
      );
    }
  };
  ...
  const allFilteredSelected = React.useMemo(() => {
    const enabled = filteredOptions
      ...
      .filter((opt) => !opt.disabled);
    return (
      enabled.length > 0 && enabled.every((opt) => selected.includes(opt.value))
    );
              ...
              groupedOptions.map((group, gi) => (
                <React.Fragment key={group.label ?? "__ungrouped__"}>
                  ...
                  {group.items.map(({ option, level, hasChildren }) => {
                    const valuesToCheck = hasChildren
                      ? collectLeafValues(option)
                      : [option.value];
                    const isSelected =
                      valuesToCheck.length > 0 &&
                      valuesToCheck.every((v) => selected.includes(v));
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "focus:bg-accent focus:text-accent-foreground relative flex w-full cursor-default items-center gap-2 rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none select-none transition-colors",
                      ...
                      >
                        ...
                        <span className='absolute right-2 flex size-3.5 items-center justify-center'>
                          {isSelected && (
                            <Iconify icon='lucide:check' className='size-3.5' />
                          )}
                        </span>
                      </button>
                    );
